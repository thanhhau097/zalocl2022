...
class LyricDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for (word_idx, word), s, e in zip(enumerate(words), starts, ends):
            ...
            if s > e:
                s, e = e, s
            multiclass_label = self.mlb.transform(
                [np.arange(int(s / max_ms * 3000), int(e / max_ms * 3000) + 1)] * len(tokens)
            )
            if len(separated_multiclass) == 0:
                separated_multiclass = multiclass_label
            else:
                separated_multiclass = np.concatenate(
                    [separated_multiclass, multiclass_label], axis=0
                )
